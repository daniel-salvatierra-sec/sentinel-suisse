"""Lombard Odier connector tests (Workday CXS, no live network)."""
...
def test_fetch_raises_when_live_disabled() -> None:
    settings = Settings(ingest_lombard_odier_live=False)
    with pytest.raises(LombardOdierDisabledError):
...
def test_parse_maps_slug_and_geneva_country(mock_get: MagicMock) -> None:
    ...
    def _side_effect(url: str, **_kwargs: object) -> MagicMock:
        ...
        assert "lombardodier.wd3.myworkdayjobs.com" in url
        return response
    ...
    assert geneva_listing.external_id == "lombard-odier-JR129879"
    assert geneva_listing.country == CountryCode.CH
    assert geneva_listing.employment_type == EmploymentType.INTERNSHIP
    assert geneva_listing.raw_payload["source"] == "lombard-odier"
...
def test_fetch_search_listings_end_to_end(mock_post: MagicMock, mock_get: MagicMock) -> None:
    ...
    def _post_side_effect(url: str, **kwargs: object) -> MagicMock:
        ...
        response.raise_for_status = MagicMock()
        assert "lombardodier.wd3.myworkdayjobs.com" in url
        assert "Lombard_Odier_Careers" in url
        call_count["n"] += 1
        if call_count["n"] == 1:
    ...
    settings = Settings(ingest_lombard_odier_live=True, ingest_rate_limit_seconds=0)
