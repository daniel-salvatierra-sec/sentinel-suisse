"""Logitech connector tests (Workday CXS, no live network)."""
...
def test_fetch_raises_when_live_disabled() -> None:
    settings = Settings(ingest_logitech_live=False)
    with pytest.raises(LogitechDisabledError):
...
def test_parse_maps_slug_and_geneva_country(mock_get: MagicMock) -> None:
    ...
    def _side_effect(url: str, **_kwargs: object) -> MagicMock:
        ...
        assert "logitech.wd5.myworkdayjobs.com" in url
        return response
...
def test_fetch_search_listings_end_to_end(mock_post: MagicMock, mock_get: MagicMock) -> None:
    ...
    settings = Settings(ingest_logitech_live=True, ingest_rate_limit_seconds=0)
