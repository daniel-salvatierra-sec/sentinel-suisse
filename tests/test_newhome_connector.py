"""newhome.ch connector parser tests (no live network)."""
...
_FIXTURE_STATE = Path(__file__).resolve().parents[1] / "fixtures" / "newhome_initial_state.json"
...
def test_parse_search_state_from_fixture() -> None:
    ...
    assert listings[0].external_id == "900111222"
    ...
    assert str(listings[0].source_url) == "https://www.newhome.ch/fr/louer/detail/900111222"
    assert listings[1].property_type == PropertyType.STUDIO


def test_fetch_raises_when_live_disabled() -> None:
    settings = Settings(ingest_newhome_live=False)
    with pytest.raises(NewhomeDisabledError):
        fetch_search_listings(settings)


@patch("sentinel_suisse.ingest.connectors.newhome.httpx.get")
def test_fetch_search_listings_when_enabled(mock_get: MagicMock) -> None:
    ...
    settings = Settings(ingest_newhome_live=True, ingest_rate_limit_seconds=0)
